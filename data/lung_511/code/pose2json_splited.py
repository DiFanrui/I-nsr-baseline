...
def make_transform_matrix(R, t):
    T = np.eye(4)
    T[:3, :3] = R
    T[:3, 3] = t / 1000 # 考虑到nsr这个baseline的像素单位是米，而我们这个数据集的单位是毫米，所以这里需要除以1000
    return T
...
def convert_to_blender_format(pose_file_path, image_dir, image_extension=".png", image_width=480):
    ...
    for idx, (t, q) in enumerate(zip(trajectory, orientations)): # type: ignore
...
convert_to_blender_format(
    pose_file_path="data/my_blender/groundtruth.txt",
    image_dir="images",
    image_extension=".png",
    image_width=480
) 
